#######
# plot variable distributions of deepntuples
# add root files with ntuples to files_mc or files_data
# itinitialize variable and bin range
#   for twodimensional variables you have to choose 'all' if you want to plot all flattened or 'max' to plot the maximum value of each tuple
#
#######
...
class datacollection:
    ...
    def fill(self):
        ...
        print 'load samples'
        self.collection_mc = rn.root2array(self.filenames_mc, "deepntuplizer/tree", list(set(self.variables)))
        self.weight_mc = rn.root2array(self.filenames_mc, "deepntuplizer/tree", "event_weight")
        if self.filenames_data != None:
            self.collection_data = rn.root2array(self.filenames_data, "deepntuplizer/tree", list(set(self.variables)))
            self.weight_data = rn.root2array(self.filenames_data, "deepntuplizer/tree", "event_weight")

        #pdb.set_trace()


    def add_variable(self,variable, bins, fillopt=''):
    ...
    def plot_variables(self):
        ...
        for i, var in enumerate(self.variables):
            print 'make plot for variable '+var
...
dc.add_variable("nCpfcand", np.arange(0,30,1))
dc.add_variable("nNpfcand", np.arange(0,30,1))
dc.add_variable("jet_pt", np.arange(0,320,20))
dc.add_variable("jet_eta", np.arange(-2.4,2.8,0.4))
dc.add_variable("npv", np.arange(0,50,1))
dc.add_variable("nsv", np.arange(0,10,1))

dc.add_variable("Cpfcan_pt", np.arange(0,105,5),fillopt='max')
dc.add_variable("Npfcan_pt", np.arange(0,105,5),fillopt='max')
dc.add_variable("sv_pt", np.arange(0,110,10),fillopt='max')

dc.add_variable("Cpfcan_pt", np.arange(0,55,2.5),fillopt='all')
dc.add_variable("Npfcan_pt", np.arange(0,55,2.5),fillopt='all')
dc.add_variable("sv_pt", np.arange(0,110,10),fillopt='all')
dc.add_variable("sv_deltaR", np.arange(0,110,10),fillopt='all')
